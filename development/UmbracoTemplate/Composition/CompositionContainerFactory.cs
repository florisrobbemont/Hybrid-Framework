...
namespace UmbracoTemplate.Composition
{
    ...
    public static class CompositionContainerFactory
    {
        ...
        public static IWindsorContainer Create()
        {
            ...
            ComponentLogger.StartLogging(windsorContainer.Kernel);

            windsorContainer.Kernel.ComponentModelBuilder.AddContributor(new LoggingModelConstruction());
            windsorContainer.Register(Component.For<LoggingInterceptor>().LifestyleTransient());
            
            windsorContainer.Install(FromAssembly.InDirectory(new ApplicationAssemblyFilter()));
