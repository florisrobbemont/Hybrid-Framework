...
namespace UmbracoTemplate.Web.Mvc
{
    ...
    public class MobileDetection
    {
        ...
        /// </summary>
        public MobileDetection(HttpContext context)
        {
            if (!string.IsNullOrEmpty(context.Request.ServerVariables["HTTP_USER_AGENT"]))
            {
                useragent = context.Request.ServerVariables["HTTP_USER_AGENT"].ToUpper();
            }
            if (!string.IsNullOrEmpty(context.Request.ServerVariables["HTTP_ACCEPT"]))
            {
